...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-white font-display flex flex-col">
      {/* 1. HEADER - KHÔI PHỤC ĐẦY ĐỦ MENU */}
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-slate-100 h-20 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-full flex items-center justify-between">
          ...
          {/* MENU DESKTOP - ĐÃ KHÔI PHỤC ĐẦY ĐỦ */}
        ...
        {/* MOBILE MENU */}
        {isMenuOpen && (
          <div className="lg:hidden fixed inset-0 top-20 bg-white z-40 p-6 flex flex-col gap-2 animate-in slide-in-from-top duration-300 overflow-y-auto pb-20">
            ...
            <div className="p-4 bg-slate-50 rounded-xl">
               <p className="text-[10px] font-black text-slate-400 uppercase mb-2">Chương trình du học</p>
               <Link onClick={toggleMenu} to="/han-quoc-2026" className="block py-2 text-sm font-bold text-[#1A531B]">Hàn Quốc 2026</Link>
               <Link onClick={toggleMenu} to="/trai-he-chantemerle" className="block py-2 text-sm font-bold text-[#F37021]">Trại hè Thụy Sĩ</Link>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 border-b border-white/10 pb-16 mb-10">
            ...
            <div className="space-y-6">
              <h4 className="text-[#F37021] font-black uppercase tracking-widest text-xs">Liên kết nhanh</h4>
              <nav className="flex flex-col gap-4 text-sm text-slate-400">
                <Link to="/partners" className="hover:text-white transition-colors">Đối tác</Link>
      ...
      {/* 3. POPUP GIẤY PHÉP - ĐÃ FIX HIỂN THỊ */}
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm">
          <div className="relative bg-white w-full max-w-2xl max-h-[90vh] rounded-[2rem] overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col">
            <div className="p-4 md:p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50 shrink-0">
              <h3 className="font-black text-[#1A531B] text-sm md:text-base uppercase">Hồ sơ pháp lý hoạt động</h3>
              <button 
                onClick={toggleModal}
            ...
            <div className="overflow-y-auto p-4 bg-white flex flex-col items-center">
              <img 
                src="/images/GiayPhepKinhDoanh.jpg" 
                alt="Bản scan giấy phép" 
                className="w-full h-auto rounded-lg shadow-sm"
                onError={(e) => {
                  e.currentTarget.src = "https://via.placeholder.com/800x1200?text=Vui+long+kiem+tra+lai+file+anh+trong+thu+muc+public/images";
                }}
              />
              <p className="mt-4 text-[10px] text-slate-400 font-bold uppercase text-center pb-4">
                Xác nhận bởi Sở Giáo dục và Đào tạo TP. Hồ Chí Minh
              </p>
            </div>
          </div>
